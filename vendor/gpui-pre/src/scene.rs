...
impl Path<Pixels> {
    ...
    /// Moves every point of the path through `f` — for a paint-time scale.
    pub fn transform_points(&mut self, f: impl Fn(Point<Pixels>) -> Point<Pixels>) {
        let origin = f(self.bounds.origin);
        let far = f(self.bounds.bottom_right());
        self.bounds = Bounds::from_corners(origin, far);
        self.start = f(self.start);
        self.current = f(self.current);
        for vertex in &mut self.vertices {
            vertex.xy_position = f(vertex.xy_position);
        }
    }
