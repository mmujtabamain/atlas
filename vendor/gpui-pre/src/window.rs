...
pub struct Window {
    ...
    /// The paint-time scales in force, outermost first: everything painted
    /// while one is in force is drawn scaled about its origin. See
    /// [`Window::with_scale`].
    pub(crate) paint_scale_stack: Vec<PaintScale>,
...
/// One paint-time scale in force: everything painted while it is on the
/// stack is drawn `scale` times its size about `origin`. Pushed by
/// [`Window::with_scale`].
#[derive(Clone, Copy, Debug, PartialEq)]
pub struct PaintScale {
    /// The factor, greater than zero.
    pub scale: f32,
    /// The point that stays where it is, in window coordinates (already
    /// moved by the scales that enclose this one).
...
impl Window {
    ...
    /// Paints everything drawn inside `f` scaled by `scale` about `origin`
    /// (a point in window coordinates), the way a CSS `transform: scale()`
    /// with a `transform-origin` does: the subtree is laid out and prepainted
    /// at its normal size, and only its painted result — boxes, borders,
    /// shadows, text, images, paths — is scaled. Layout, hitboxes and scroll
    /// geometry are untouched, so a scaled subtree's controls still respond
    /// where they were laid out; a caller that scales a subtree visibly should
    /// block its input for the duration.
    ///
    /// Scales nest: an inner scale composes with the outer ones. Content
    /// masks pushed inside the scope scale with their content; masks pushed
    /// outside it keep clipping where they were.
    ///
    ...
    pub fn with_scale<R>(
        ...
        if !scale.is_finite() || scale <= 0. || (scale - 1.).abs() < f32::EPSILON {
            return f(self);
        }
        // The origin is a point in the untransformed window; the outer scales
        // already in force move it like any other point.
        let origin = self.transform_point(origin);
        self.paint_scale_stack.push(PaintScale { scale, origin });
        let result = f(self);
        self.paint_scale_stack.pop();
        result
    }

    /// The product of the paint-time scales in force.
    pub fn paint_scale(&self) -> f32 {
        self.paint_scale_stack.iter().map(|scale| scale.scale).product()
    }

    /// Whether a paint-time scale is in force.
    pub fn is_paint_scaled(&self) -> bool {
        !self.paint_scale_stack.is_empty()
    }

    /// A point in the untransformed window, where the scales in force draw it.
    pub fn transform_point(&self, point: Point<Pixels>) -> Point<Pixels> {
        // The innermost scale applies first, then each enclosing one.
        self.paint_scale_stack
            .iter()
            .rev()
            .fold(point, |point, scale| scale.origin + (point - scale.origin).map(|c| c * scale.scale))
    }

    /// A rectangle in the untransformed window, where the scales in force draw it.
    pub fn transform_bounds(&self, bounds: Bounds<Pixels>) -> Bounds<Pixels> {
        if self.paint_scale_stack.is_empty() {
            return bounds;
        }
        let origin = self.transform_point(bounds.origin);
        let far = self.transform_point(bounds.bottom_right());
        Bounds::from_corners(origin, far)
    }

    /// A length in the untransformed window, as the scales in force draw it.
    pub fn transform_length(&self, length: Pixels) -> Pixels {
        if self.paint_scale_stack.is_empty() {
            return length;
        }
        length * self.paint_scale()
    }
    ...
    pub fn paint_drop_shadows(
        ...
        let bounds = self.transform_bounds(bounds);
        let corner_radii = corner_radii.map(|radius| *radius * paint_scale);
        ...
        for shadow in shadows {
            ...
            let shadow_bounds = (bounds + shadow.offset.map(|c| c * paint_scale))
                .dilate(shadow.spread_radius * paint_scale);
            self.next_frame.scene.insert_primitive(Shadow {
                order: 0,
                blur_radius: (shadow.blur_radius * paint_scale).scale(scale_factor),
    ...
    pub fn paint_inset_shadows(
        ...
        for shadow in shadows {
            ...
            let spread = shadow.spread_radius * paint_scale;
            let hole = (bounds + shadow.offset.map(|c| c * paint_scale)).dilate(-spread);
            ...
            let hole_corner_radii = Corners {
                top_left: (corner_radii.top_left - spread).max(zero),
                top_right: (corner_radii.top_right - spread).max(zero),
                bottom_right: (corner_radii.bottom_right - spread).max(zero),
                bottom_left: (corner_radii.bottom_left - spread).max(zero),
            };
            self.next_frame.scene.insert_primitive(Shadow {
                order: 0,
                blur_radius: (shadow.blur_radius * paint_scale).scale(scale_factor),
    ...
    pub fn paint_quad(&mut self, quad: PaintQuad) {
        ...
        let opacity = self.element_opacity();
        let paint_scale = self.paint_scale();
        let quad = if self.is_paint_scaled() {
            PaintQuad {
                bounds: self.transform_bounds(quad.bounds),
                corner_radii: quad.corner_radii.map(|radius| *radius * paint_scale),
                border_widths: quad.border_widths.map(|width| *width * paint_scale),
                ..quad
            }
        } else {
            quad
    ...
    pub fn paint_path(&mut self, mut path: Path<Pixels>, color: impl Into<Background>) {
        ...
        let scale_factor = self.scale_factor();
        if self.is_paint_scaled() {
            let scales: Vec<PaintScale> = self.paint_scale_stack.clone();
            path.transform_points(|point| {
                scales
                    .iter()
                    .rev()
                    .fold(point, |point, scale| scale.origin + (point - scale.origin).map(|c| c * scale.scale))
    ...
    pub fn paint_underline(
        ...
        let thickness = self.snap_stroke(self.transform_length(style.thickness));
    ...
    pub fn paint_glyph(
        ...
        let element_opacity = self.element_opacity();
        let scale_factor = self.scale_factor();
        // A scaled glyph is rasterized at the scaled size, so it stays crisp.
        let origin = self.transform_point(origin);
        let font_size = self.transform_length(font_size);
    ...
    pub fn paint_svg(
        ...
        let element_opacity = self.element_opacity();
        let bounds = self.snap_bounds(self.transform_bounds(bounds));
    ...
    pub fn paint_image(
        ...
        let bounds = self.transform_bounds(bounds);
        let image_bounds = self.transform_bounds(image_bounds);
        let corner_radii = corner_radii.map(|radius| self.transform_length(*radius));
    ...
    pub fn dispatch_event(&mut self, event: PlatformInput, cx: &mut App) -> DispatchEventResult {
        ...
        self.last_input_modality = match &event {
            ...
            // A mouse-up is mouse input as much as a move or a down: a key
            // pressed in the middle of a drag (to cycle a drop target, say)
            // must not make the release that follows miss its target.
            PlatformInput::MouseMove(_) | PlatformInput::MouseDown(_) | PlatformInput::MouseUp(_) => InputModality::Mouse,
