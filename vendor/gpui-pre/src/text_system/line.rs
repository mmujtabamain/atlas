    ...
    window.paint_layer(line_bounds, |window| {
        ...
        for (run_ix, run) in layout.runs.iter().enumerate() {
            ...
            for (glyph_ix, glyph) in run.glyphs.iter().enumerate() {
                ...
                // The mask is where things are painted; a scale in force
                // moves the glyph there too before the two are compared.
                let content_mask = window.content_mask();
                if window.transform_bounds(max_glyph_bounds).intersects(&content_mask.bounds) {
